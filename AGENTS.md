# Global Agents Guidance (./AGENTS.md)
...
# What
## Mission
copier template to easily kickstart new python projects with basic ci/cd setup:
1. project directory format
2. dependency manager
3. testing and linting
Projects repositories that use this template should be able to update their current repository by pulling the new changes from this copier template.

## project structure
./src/ contains the source code
./test/ contains the test code files
./docs/ contains the resources such as images for readme.
...
# Web search policy
- Disable web search to avoid massive token consumption
- If web searching, only fetch the minimal target docs; summarize (no large dumps)
No fallback code when primary method fails. Always 

# Default autonomy and safety
- Default to read-only exploration and analysis.
- When edits are needed, only **workspace-scoped** write access and keep changes inside the repo.
- When interacting with remote APIs, you must use READ-only calls, unless explicitily instructed otherwise by the user. 
